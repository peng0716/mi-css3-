...
(function () {
    ...
    _utils.getElementsByClass = function getElementsByClass(strClass, context) {
        ...
        //To determine whether the current browser is compatible with our getElementsByClassName approach, we can use this method directly, in the context of the implementation of context（确定当前的浏览器getElementsByClassName方法兼容，我们可以直接使用这个方法）
        if ("getElementsByClassName" in document) {
            return this.listToArray(context.getElementsByClassName(strClass));
        }
        //In case of not compatible with this method we have to write code to handle compatible（在不兼容的情况下，我们必须编写代码来处理兼容）
